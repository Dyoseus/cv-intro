...
def detect_lines(img, threshold1 = 50, threshold2 = 70, apertureSize = 3, minLineLength = 100, maxLineGap = 50):
...
def detect_lanes(imageInput, lines):
    ...
    for i in range(1, len(lineDict)):
        ...
        if abs(intercept1 - intercept2) < 500 and abs(intercept1 - intercept2) > 10 and abs(slope1 - slope2) < 6:
        # and abs(slope1 - slope2) > 0.03:
        ...
        elif abs(intercept1 - intercept2) < 500 and abs(intercept1 - intercept2) > 10 and abs(-slope1 - slope2) < 6:
            index1, index2 = intercepts.index(list(lineDict)[i-1]), intercepts.index(list(lineDict)[i])
            line1, line2 = lines[index1], lines[index2]
            possibleLanes.append([line1, line2])
