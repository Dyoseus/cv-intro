...
import LincolnLabsSecretSauce as lane_detection_Emily
...
def get_lane_center(img, lanes):
    ...
    if lanes is not None:
        for lane in lanes:
            slope, intercept = lane_detection_Emily.get_slopes_intercepts(img, lane)
...
def videoDetection(vid):
    ...
    while count <= 500:
        ...
        if ret:
            ...
            lines = lane_detection_Emily.detect_lines(againResized, 50, 70, 3, 200, 10)
            if lines is not None:
                lanes = lane_detection_Emily.detect_lanes(againResized, lines)   
            else:
                lanes = []
            if len(lanes) != 0:
            ...
            againResized = lane_detection_Emily.draw_lanes(againResized, lanes)
    ...
    while count <= 500:
        if ret:
            ...
            if len(detected_tags) != 0:
                ...
                lines = lane_detection_Emily.detect_lines(frame, 50, 70, 3, 200, 10)
                if lines is not None:
                    lanes = lane_detection_Emily.detect_lanes(frame, lines)   
                else:
                    lanes = []
                if len(lanes) != 0:
            ...
            frame = lane_detection_Emily.draw_lanes(frame, lanes)
            # total_frames.append(frame)
...
def videoDetectionFrames(vid, framesVid):
    ...
    while True:
        ...
        if ret:
            ...
            lines = lane_detection_Emily.detect_lines(againResized, 50, 50, 3, 100, 10)
            if lines is not None:
                lanes = lane_detection_Emily.detect_lanes(againResized, lines)   
            else:
                lanes = []
...
if __name__ == "__main__":
    ...
    lines = lane_detection_Emily.detect_lines(img, 30, 100, 3, 229, 13)
    lanes = lane_detection_Emily.detect_lanes(img, lines)
